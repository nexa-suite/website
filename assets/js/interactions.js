document.addEventListener('DOMContentLoaded', () => {
  const isLocalStack = ['localhost', '127.0.0.1'].includes(window.location.hostname);
  const FALLBACK_WEBAPP_BASE = isLocalStack
    ? 'http://localhost:5173'
    : 'https://nexa-webapp.onrender.com';
  const FALLBACK_PORTAL_BASE = isLocalStack
    ? 'http://localhost:5174'
  ...
  if (isLocalStack) {
    document.querySelector('#nav-platform')?.setAttribute('href', `${webappBase}/#/auth/login`);
    document.querySelector('#nav-buyer-portal')?.setAttribute('href', `${FALLBACK_PORTAL_BASE}/#/auth/login`);
    document.querySelector('#nav-login-buyer')?.setAttribute('href', `${FALLBACK_PORTAL_BASE}/#/auth/login`);
  }
